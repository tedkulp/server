...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        this.portRange = (options.portRange && options.portRange.split('-')) || undefined;
    ...
    listen() {
        ...
        return new Promise((resolve) => {
            ...
            if (this.portRange) {
                const start = parseInt(this.portRange[0]);
                const end = parseInt(this.portRange[1]);
                random_port({from: start, range: (end - start) + 1}, freePort => {
                    server.listen(freePort, cb);
                });
            } else {
                server.listen(cb);
            }
